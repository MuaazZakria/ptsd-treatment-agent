...
Runs the deterministic pipeline stages in code (safety gate -> rule-based queries ->
...
def citation_check(options: list[dict], retrieved_ids: set[str]) -> dict:
    """Mirrors the original pipeline's audit_node (deterministic)."""
    used: list[str] = []
    uncited: list[str] = []
    for o in options or []:
        ids = [x for x in (o.get("ev") or []) if x]
...
def offline_stub(patient_compact: dict, compact_ev: list[dict]) -> dict:
    """Deterministic placeholder - mirrors the original pipeline's offline stub."""
